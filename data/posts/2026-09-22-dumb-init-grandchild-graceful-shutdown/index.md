...
전파 시간을 벌어주는 것이 없으므로 SIGTERM 이 endpoint 제외보다 먼저 도착할 수 있다 \
...
`exec` 를 적용한 서비스에 15초 동안 유지되는 RPC 를 일부러 걸어둔 채로 pod 를 정상 삭제했더니 RPC 는 15.03초를 채우고 exit 0 으로 끝났고 pod 삭제에는 17.93초가 걸렸다 \
